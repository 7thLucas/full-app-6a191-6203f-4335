...
> Module for connecting to the audio analyzer microservice — Express routes, server client, React hooks, and composable UI components for audio/video transcription and AI-powered quality analysis.
...
`@qb/audio-analyzer` is a scaffold module that wires an Express backend to the `qb-micro-audio-analyzer-service` microservice and exposes a React-friendly surface for the frontend. The server side proxies multipart file uploads, job polling, and asset streaming. The client side provides hooks and composable components that drive the full transcription-and-analysis UX without any direct microservice calls from the browser.
...
- **`TranscriptionUpload` component** — drag-and-drop / click-to-browse file picker with loading, dragging, and validation states.
...
| `POST` | `/api/audio-analyzer/transcribe` | `postTranscribe` | Accepts `multipart/form-data` with one or more `files` fields and an optional JSON `analysis_options` field. Queues the job and returns `{ ticket_id }`. |
...
4. **Customize analysis options** — edit `src/constants/default-analysis-options.ts` to set your domain context, speaker roles (`staff`, `customer`, …), and scoring rules. These defaults are applied on every transcription job unless the caller provides an override in the `analysis_options` form field.

5. **Import client-side APIs from the package root only** — all browser-safe exports are available from `@qb/audio-analyzer`. Never import `audio-analyzer.ts`, routes, or the controller from client-side code; import those directly on the server.

6. **Render only API-available data** — see `src/RULES.md` for the full data contract. Do not display staff profile names/photos, review dates, waveforms, or any field not present in `TrackTranscribeResult` / `AnalysisResult`.
